...
#define PIN_TCRT5000_SENSOR     A1  // TCRT5000 IR sensor for plastic detection (Analog)
...
const int TCRT5000_DETECTION_THRESHOLD = 500;   // Analog threshold for IR detection (lower = object detected)

// Double-Check Detection Configuration
const unsigned long DOUBLE_CHECK_INTERVAL = 100;  // Interval between first and second detection (ms)
const int DOUBLE_CHECK_REQUIRED_COUNT = 2;        // Number of consistent detections required
...
// Double-Check Detection State
MaterialType pendingMaterial = MATERIAL_NONE;
...
MaterialType detectMaterial() {
  // Double-check detection algorithm
  // Requires consistent detection twice before confirming material
  
  unsigned long currentMillis = millis();
  MaterialType currentDetection = detectMaterialRaw();
  
  if (currentDetection == MATERIAL_NONE) {
    // No material detected, reset the detection state
    pendingMaterial = MATERIAL_NONE;
    detectionCount = 0;
    return MATERIAL_NONE;
  }
  
  // Check if this is the same material as pending detection
  if (currentDetection == pendingMaterial) {
    // Check if enough time has passed since last detection
    if (currentMillis - lastDetectionTime >= DOUBLE_CHECK_INTERVAL) {
      detectionCount++;
      ...
      Serial.print(F("/"));
      Serial.println(DOUBLE_CHECK_REQUIRED_COUNT);
      
      // Check if we have enough consistent detections
      if (detectionCount >= DOUBLE_CHECK_REQUIRED_COUNT) {
        Serial.print(F("Material CONFIRMED: "));
    ...
    // Different material detected, start new detection sequence
    pendingMaterial = currentDetection;
    detectionCount = 1;
    lastDetectionTime = currentMillis;
    
    Serial.print(F("New material pending: "));
    switch (currentDetection) {
      case MATERIAL_METAL: Serial.println(F("METAL")); break;
      case MATERIAL_PLASTIC: Serial.println(F("PLASTIC")); break;
      case MATERIAL_BIODEGRADABLE: Serial.println(F("BIODEGRADABLE")); break;
...
MaterialType detectMaterialRaw() {
  // Raw single detection - checks all sensors once
  // Priority: Check for biodegradable first, then metal, then plastic (via IR)
...
bool isPlasticDetected() {
  // Plastic detection using TCRT5000 IR sensor
  // If IR sensor detects object but metal, capacitive, and gas sensors do NOT trigger,
  // then it must be a plastic material
  ...
  return irDetected && !metalDetected && !capacitiveDetected && !gasDetected;
}

bool isIRObjectDetected() {
  // TCRT5000 IR sensor: Lower analog value means object is closer/detected
  int irValue = analogRead(PIN_TCRT5000_SENSOR);
  return irValue < TCRT5000_DETECTION_THRESHOLD;
}
